...
public class Agent_Regression extends Agent_ComputingAgent {
	 ...
	 */
	private double modelError(Instances insts) throws Exception{
		if(coefs.size()==0){
		...
		return Math.sqrt(e/insts.numInstances());
	}
	...
	protected Evaluation test() {
		 ...
		 System.out.println("Agent "+getLocalName()+": Testing...");
			...
			try {
				MyWekaEvaluation result = new MyWekaEvaluation(modelError(test),0.0);
